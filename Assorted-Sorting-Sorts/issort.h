...
void merge_sort(int a[], int n, int (*cmp)()) {
  ...
  while(i < nL || j < nR) { /* while non-empty left/right subarrays */
    if(i < nL && (j >= nR || (*cmp)(L[i],R[j]) > 0)) 
      a[k++] = L[i++]; /* put L value or empty R array */
    else 
      a[k++] = R[j++]; /* put R value or empty L array */
  }
}
static void recursive_mergessort(int len, char m[][len], int n, int (*cmp)()) {
  ...
  while(i < nL || j < nR) { /* while non-empty left/right subarrays */
    if(i < nL && (j >= nR || (*cmp)(L[i],R[j]) > 0)) 
      strcpy(m[k++], L[i++]); /* put L value or empty R array */
    else
      strcpy(m[k++], R[j++]); /* put R value or empty L array */
  }
}
...
void hash_sort(int a[], int n, int (*cmp)()) {/* O(R) == O(max-min) */
  ...
  inc = ((*cmp)(1,3)>0&&(*cmp)(2,4)>0&&(*cmp)(1,6)>0&&(*cmp)(2,5)>0) ? 1 : -1; /* ascend/descend */
  i = (inc == -1) ? (range - 1) : 0;
